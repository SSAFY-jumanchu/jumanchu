"""뉴스 섹터 추출 — 순수 Python 알고리즘 모듈 (Algo 담당: 정율)
================================================================
역할: 뉴스 텍스트(제목+요약)에서 **종목을 태깅**하고, 그 종목의 업종/섹터
      (DB `Stock.sector`)를 역산해 **뉴스의 섹터 태그**를 만든다.

설계 결정(2026-06-10) — "왜 종목→섹터인가":
  - DB는 종목마다 `Stock.sector`를 이미 보유하고, 사용자 `InvestmentProfile
    .preferred_sector`가 같은 어휘로 매칭된다. 뉴스 섹터를 종목의 sector에서
    역산하면 **어휘가 자동 일치** → 추천(궁합 점수)에 바로 사용 가능.
  - 키워드 사전을 따로 만들면 라벨이 Stock.sector와 어긋나 추천 매칭이 깨지고
    사전 유지보수 부담이 생긴다.
  - `StockNews`엔 섹터 컬럼이 없지만 `NewsRelatedStock → Stock.sector` 조인으로
    조회 시점 역산이 가능하므로 **모델 변경 불필요**.

이 모듈은 Django에 의존하지 않는다(순수 함수). BE는 DB에서 종목 목록을 읽어
`StockRef` 리스트로 넘기고, 반환된 섹터 태그를 NewsRelatedStock 저장/조회에 쓴다.

BE ↔ Algo 인터페이스 (코드 작성 전 합의 대상):
...
# --------------------------------------------------------------------------- #
# 종목 태깅 (NEWS_API_SPEC §3-3 로직을 sector 산출용으로 정리)
# --------------------------------------------------------------------------- #
...
def extract_sectors(
    ...
    *,
    ...
    Args:
        ...
        aliases: 약어/별칭 → 종목코드 리스트. 기본은 `COMMON_KR_ALIASES`.
            한국 뉴스의 "삼전·하닉·엔솔" 등 정식명 외 표기를 잡는다. None이면 비활성.
