...
function finish() {
  localStorage.setItem('wf_onboarded', '1')
  router.push('/')
}
  ...
  <div class="ob-shell">
    ...
    <div v-if="step === 7" class="ob-result-wrap">
      <div class="panel ob-result-card">
        ...
        <button class="primary-btn" @click="finish">주만추 시작하기 🎉</button>
      </div>
    </div>
...
.ob-progress-track {
  height: 5px;
  background: rgba(200, 215, 255, 0.45);
  border-radius: 999px;
  overflow: hidden;
}
...
.ob-option.is-selected .ob-option-desc { color: rgba(49, 93, 255, 0.65); }
