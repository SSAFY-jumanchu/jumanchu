...
const change = stock.price - stock.prevClose        // +7,500
const changeRate = (change / stock.prevClose) * 100  // +2.42%
...
const chartPath = computed(() => buildPath(intradayPrices, 560, 200))
...
const maxAskQty = computed(() => Math.max(...asks.map(a => a.qty)))
const maxBidQty = computed(() => Math.max(...bids.map(b => b.qty)))
...
function setQtyPct(pct) {
  // 와이어프레임용: 1,000만원 보유 가정
  const budget = 10000000
  orderQty.value = Math.floor((budget * pct) / orderPrice.value)
}
      ...
      <div class="sd-header-body">
        ...
        <div class="sd-title-block">
          ...
          <div class="sd-price-row">
            ...
            <span class="sd-change is-up">
              +₩{{ fmt(change) }} (+{{ changeRate.toFixed(2) }}%)
            </span>
          </div>
        </div>
      ...
      <div class="sd-col-right">
        ...
        <div class="panel order-panel">
          ...
          >
            ...
            <small>주문 API 연결 예정</small>
          </button>

          <p class="fine-print">
            실제 체결은 <code>/orders/preview/</code> 검증 뒤<br>
