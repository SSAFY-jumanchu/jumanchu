...
import { fetchWatchlist, addWatchlist, removeWatchlist } from '../api/recommend'
import { errMsg } from '../api/client'

const router = useRouter()

// ===== 추천 후보 브라우즈 리스트 (큐레이션 목업 — 후보 엔드포인트 없음) =====
const stocks = [
  {
...
// ===== 관심종목 (실데이터: GET /watchlist/) =====
const wlItems = ref([]) // 백엔드 원본 항목
const wlError = ref('')

const watchedCodes = computed(() => wlItems.value.map((i) => i.stock_code))
const watchlist = computed(() =>
  wlItems.value.map((i) => ({
    ...
    market: i.market,
    sector: i.sector,
    currency: currencyOf(i.market),
    // 목록 응답은 current_price를 null로 줌(라이브 호출 생략) → null 유지해 '—' 표시
    price: i.current_price != null ? Number(i.current_price) : null,
  })),
)
...
async function loadWatchlist() {
  try {
    ...
    wlError.value = errMsg(e)
  }
}

async function toggleWatch(code) {
  try {
    if (watchedCodes.value.includes(code)) await removeWatchlist(code)
...
// ===== 선택 종목 상세 (실데이터: detail + price + financials + chart) =====
const selectedCode = ref('005930')
const selectedStock = ref({
  code: '005930', name: '삼성전자', market: 'KOSPI', sector: '전기·전자', currency: 'KRW',
  price: 0, open: 0, high: 0, low: 0, volume: 0,
  beta: 0, volatility: 0, roe: null, high52w: 0, low52w: 0,
  sparkline: [50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50],
})

async function loadSelected(code) {
  try {
    const [detail, priceRes, finRes, chartRes] = await Promise.all([
      fetchStockDetail(code),
      fetchStockPrice(code).catch(() => null),
      fetchStockFinancials(code).catch(() => null),
      fetchStockChart(code, { period: '1y', interval: '1d' }).catch(() => null),
    ])
    const d = detail.stock
    const p = priceRes?.price
    const ind = finRes?.indicator || {}
    ...
    selectedStock.value = {
      ...
      market: d.market,
      sector: d.sector,
      currency: d.currency || currencyOf(d.market),
      price: p ? Number(p.current) : 0,
      open: p ? Number(p.open) : 0,
      high: p ? Number(p.high) : 0,
      low: p ? Number(p.low) : 0,
      volume: p ? Number(p.volume) : 0,
      beta: ind.beta != null ? Number(ind.beta) : 0,
      volatility: ind.volatility != null ? Number(ind.volatility) : 0,
      roe: ind.roe != null ? +(ind.roe * 100).toFixed(2) : null,
      high52w: ind.high_52w ?? 0,
      low52w: ind.low_52w ?? 0,
      sparkline: candles.length ? candles.slice(-30).map((c) => Number(c.close)) : [50, 50, 50, 50, 50, 50],
    }
  } catch {
    // 실패 시 기존 표시 유지
  }
...
watch(selectedCode, (code) => loadSelected(code))
...
onMounted(() => {
  loadWatchlist()
  loadSelected(selectedCode.value)
})
</script>
